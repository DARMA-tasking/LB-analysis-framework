...
from lbaf.IO.lbsStatistics import initialize, print_function_statistics, Hamming_distance
...
class internalParameters:
    ...
    def get_conf_file(self, conf_file: str):
    ...
    def checks_after_init(self):
        ...
        if self.log_file is not None:
            ...
            if os.path.isdir(os.path.abspath(os.path.split(self.log_file)[0])):
                ...
                print(self.log_file)
                self.logger.error(f"LOG directory in {self.log_file} NOT FOUND!")
                sys.exit(1)
...
class LBAFApp:
    ...
    def main(self):
        ...
        sv = sys.version_info
        self.logger.info(f"### Started with Python {sv.major}.{sv.minor}.{sv.micro}")
        ...
        phase = Phase(0, logger=self.logger, file_suffix=self.params.file_suffix)
        if self.params.log_file:
            # Populate phase from log files and store number of objects
            n_o = phase.populate_from_log(n_ranks, self.params.phase_id, self.params.log_file)
        else:
            # Populate phase pseudo-randomly
            phase.populate_from_samplers(
                self.params.n_objects,
                self.params.time_sampler_type,
                self.params.time_sampler_parameters,
                self.params.communication_degree,
                self.params.volume_sampler_type,
                self.params.volume_sampler_parameters,
                n_ranks,
                self.params.n_mapped_ranks)

            # Keep track of number of objects
            n_o = self.params.n_objects
        ...
        if self.params.brute_force_optimization:
        ...
        if self.params.generate_multimedia:
