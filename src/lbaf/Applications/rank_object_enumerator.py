...
from lbaf.IO.lbsStatistics import (
    compute_arrangement_works,
    compute_min_max_arrangements_work,
    compute_pairwise_reachable_arrangements
)
...
def main():
    ...
    file_prefix = f"{os.sep}".join(
        [data_dir, file_prefix])  # make absolute path prefix

    reader = LoadReader(file_prefix=file_prefix,
                        logger=root_logger, file_suffix=file_suffix)
    phases = {}
    ...
    if n_a != n_ranks ** len(objects):
        ...
        raise SystemExit(1)
    root_logger.info(
        f"Number of generated arrangements with repetition: {n_a}")
    root_logger.info(
        f"\tminimax work: {w_min_max:.4g} for {len(a_min_max)} optimal arrangements")
    ...
    with open(out_name, 'w', encoding="utf-8") as input_file:
        ...
        for a in a_min_max:
            writer.writerow(a)
    root_logger.info(
        f"Wrote {len(a_min_max)} optimal arrangement to {out_name}")
    ...
    initial_works = compute_arrangement_works(
        objects, initial_arrangement, alpha_g, beta_g, gamma_g)
